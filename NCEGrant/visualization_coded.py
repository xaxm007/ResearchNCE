...
#printing caption: for PIL to draw the caption to video
def paint_text(im, cap, font, pos, color):
    ...
    draw.text(position, cap, font=font, fill=fillColor)
...
def processImg(img, cur_time, title, dense_captions, prop_idx, n_caption=3, output_language='en'):
    ...
    #blending two images 'img' and 'bg_img' based on 'mask' with variable transparency 'alpha'
    mask = bg_img / 255.
    alpha = 0.5
    ...
    if output_language == 'ne':
        font = ImageFont.truetype('visualization/himali.ttf', font_size)
    else:
...
def vid_show(vid_path, captions, save_mp4, save_mp4_path, output_language='en'):
    ...
    if output_language != 'en':
        for proposal in captions:
            caption = GoogleTranslator(sorce='en', target=output_language).translate(proposal['sentence'])

            #translated caption replaces original caption
            proposal['sentence'] = caption
    for i, proposal in enumerate(captions):
        #add original key id to caption
    ...
    for frame_id in tqdm(range(int(frame_count))):
        ...
        if opt.show_all_caption_per_frame:
            ...
            #no. of captions to show in video
            n_caption = min(1, len(captions_to_show))
        
        title = '{:.1f}s/{:.1f}s'.format(frame_time, duration)

        #print caption in terminal
        proposal = frame_captions[0]
        print(f"Caption: {proposal['sentence']}")

        frame, cur_time, imp_fps = processImg(frame, cur_time, title, captions_to_show, highlight_idx, output_language=output_language, n_caption=n_caption)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_mp4_folder', type=str)
    ...
    for vid, dense_captions in d.items():
        if opt.output_mp4_folder is None:
            opt.output_mp4_foler = opt.input_mp4_folder + '_output'
        ...
        if not os.path.exists(input_mp4_path):
            print('video {} does not exit so SKIP.')
            continue
