...
#setting the timesatmp for caption
...
def vid_show(vid_path, captions, save_mp4, save_mp4_path, output_language='en'):
    ...
    for i, proposal in enumerate(captions):
        ...
        #add original key id to captions
    ...
    for frame_id in tqdm(range(int(frame_count))):
        ...
        # Print the captions in the terminal
        # print(f"Timestamp: {frame_time:.1f}s")
        # for proposal in frame_captions:
        #     if frame_captions:
        # Select the first caption for the current time interval
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_language', type=str, default='en',
                        help='Refer to https://py-googletrans.readthedocs.io/en/latest/#googletrans-languages for more information')
    ...
    for vid, dense_captions in d.items():
        ...
        if not os.path.exists(input_mp4_path):
            print('video {} does not exist, skip it.')
            continue
